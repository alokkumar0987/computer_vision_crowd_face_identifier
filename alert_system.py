...
class AlertSystem:
    ...
    def trigger_alert(self, camera_id: int, camera_name: str, face_name: str, face: Face, confidence: float, frame: np.ndarray) -> AlertEvent:
        ...
        if self.alert_enabled:
            ...
            self._play_voice_alert(face_name)
    ...
    def _play_voice_alert(self, name: str) -> None:
        """Play voice alert saying the person's name"""
        if self.tts_engine:
            try:
                voice_thread = threading.Thread(target=self._speak, args=(f"{name} is detected",))
                voice_thread.start()
            except Exception as e:
                logger.error(f"Error starting voice alert thread: {e}")
    ...
    def _speak(self, text: str) -> None:
        try:
            ...
            logger.error(f"Error in TTS speech: {e}")
